...
EOT;
        ...
        if ($countUrl == 3) {
            $controller = parse_name($urlArr[1], 1);
            $action     = $urlArr[2];
            $appName    = $urlArr[0];
    ...
    protected function discoverCliGroups()
    {
        $cliGroups = [];
        $appPath   = app_path();
        $apps      = scan_dir($appPath . '*', GLOB_ONLYDIR);
        foreach ($apps as $app) {
            $cliFiles = scan_dir($appPath . "$app/cli/*Cli.php");
            foreach ($cliFiles as $cliFile) {
                $cliClassName      = str_replace('.php', '', $cliFile);
                $cliControllerName = parse_name(str_replace('Cli', '', $cliClassName));
                ...
                if (class_exists($class)) {
                    ...
                    foreach ($methods as $method) {
                        ...
                        if ($method->isPublic()) {
                            ...
                            try {
                                $attrs = $method->getAttributes(\rolink\console\Cli::class);
                                /* @var \rolink\console\Cli $mCli */
                                $mCli = $attrs[0]->newInstance();
                            } catch (\Exception $e) {
                                $mCli = new \rolink\console\Cli('');
                            }
    ...
    protected function describeCliHelp($cliName, $cliClass, $action)
    {
        ...
        $attrs   = $reflect->getAttributes(\rolink\console\Cli::class);
